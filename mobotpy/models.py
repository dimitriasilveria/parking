...
class Articulated:
    ...
    def f(self, x, u):
        """Articulated bicycle vehicle kinematic model.
        ...
        f[0] = u[0] * np.cos(x[2])
        f[1] = u[0] * np.sin(x[2])
        f[2] = (u[0] * np.sin(x[3]) + u[1] * self.ell_T_r) / (self.ell_T_r + self.ell_T_f * np.cos(x[3]))
        f[3] = u[1]
        return f
    ...
    def animate(
        ...
        T,
        save_ani=False,
        filename="animate_articulated.gif",
    ):
        """Create an animation of a tricycle vehicle.
        ...
        (rightwheel,) = ax.fill([], [], color="k")
        (body,) = ax.fill([], [], color="C0", alpha=0.5)
        (line_rear,) = ax.plot([], [], "C0")
        (leftwheel_rear,) = ax.fill([], [], color="k")
        (rightwheel_rear,) = ax.fill([], [], color="k")
        (body_rear,) = ax.fill([], [], color="C0", alpha=0.5)
        time_text = ax.text(0.05, 0.9, "", transform=ax.transAxes)
        ...
        def init():
            ...
            body.set_xy(np.empty([5, 2]))
            line_rear.set_data([], [])
            leftwheel_rear.set_xy(np.empty([5, 2]))
            rightwheel_rear.set_xy(np.empty([5, 2]))
            body_rear.set_xy(np.empty([5, 2]))
            time_text.set_text("")
            return line, leftwheel, rightwheel, body, line_rear, leftwheel_rear, rightwheel_rear, body_rear, time_text
        ...
        def movie(k):
            ...
            rightwheel.set_xy(np.transpose([X_R, Y_R]))
            body.set_xy(np.transpose([X_B, Y_B]))
            leftwheel_rear.set_xy(np.transpose([X_LR, Y_LR]))
            rightwheel_rear.set_xy(np.transpose([X_RR, Y_RR]))
            body_rear.set_xy(np.transpose([X_BR, Y_BR]))
            # Add the simulation time
            time_text.set_text(r"$t$ = %.1f s" % (k * T))
            # Dynamically set the axis limits
            ax.set_xlim(x[0, k] - 10 * self.ell_W_r, x[0, k] + 10 * self.ell_W_f)
            ax.set_ylim(x[1, k] - 10 * self.ell_W_r, x[1, k] + 10 * self.ell_W_f)
            ax.figure.canvas.draw()
            # Return the objects to animate
            return line, leftwheel, rightwheel, frontwheel, body, line_rear, leftwheel_rear, rightwheel_rear, frontwheel_rear, body_rear, time_text
