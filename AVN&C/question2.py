...
for k in range(1, N):
    ...
    A = np.array(
        [
            [0, V * np.cos(x_d[2, k - 1])],
            [0, 0],
        ]
    )
    B = np.array([[0.5*np.sin(x_d[2, k - 1]), 0.5*np.sin(x_d[2, k - 1])], [-1/ELL, 1/ELL]])
    O = np.concatenate(([B,(A@B)]),axis=1)
    ...
    u[:, k] = -K.gain_matrix @ (e[:, k - 1]) + u_d[:, k - 1]
...
plt.plot(t,x[1, :]- x_d[1, :], "C1--")
#plt.plot(t, x[1, :], "C0")
plt.grid(color="0.95")
plt.ylabel(r"$y$ [m]")
plt.setp(ax1b, xticklabels=[])
ax1c = plt.subplot(413)
plt.plot(t, x[2, :] - x_d[2, :] * 180.0 / np.pi, "C1--")
